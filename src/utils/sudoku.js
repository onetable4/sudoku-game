...
function countSolutions(grid, count = { value: 0 }) {
  if (count.value >= 2) return count.value; // Stop if more than one solution found
...
function hasUniqueSolution(puzzle) {
  ...
  const solutions = countSolutions(gridCopy);
  return solutions === 1;
}
  ...
  for (const pos of positions) {
    ...
    if (puzzle[row][col] !== 0) {
      ...
      if (hasUniqueSolution(puzzle)) {
        ...
        // Restore the number - removing it creates multiple solutions
...
// Check if the puzzle is completely filled without conflicts
